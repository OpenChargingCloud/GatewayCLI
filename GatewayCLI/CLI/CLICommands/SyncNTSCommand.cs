...
namespace cloud.charging.open.Gateway.CommandLine
{
    ...
    /// <summary>
    /// Ask this gateway's time servers what the time is, or one of them
    /// everything.
    /// </summary>
    /// <remarks>
    /// Without a server, the same thing the NTS page does with 'Sync now'. Both
    /// end in <see cref="Gateway.SyncTimeAsync"/>, which writes every step into the
    ...
    /// <see cref="Gateway.TestTimeServerAsync"/>, on the ports the server is
    /// configured with, step by step. Only a server of this gateway is
    ...
    /// </remarks>
    /// <param name="CLI">The command line of the gateway to ask.</param>
    public class SyncNTSCommand(GatewayCLI CLI) : ACLICommand<GatewayCLI>(CLI),
                                                  ICLICommand
    {
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length == 2)
            {
                ...
                var server  = cli.Gateway.TimeSources.Sources.FirstOrDefault(source => String.Equals(source.Hostname.Trimmed,
                                                                                                     wanted,
                ...
                if (server is null)
                    return [ $"'{Arguments[1]}' is none of this gateway's time servers, which are {String.Join(", ", TimeServers())}." ];
                ...
                cli.Gateway.Log.Info(
                    $"Somebody at the command line asked this gateway to test the time server '{host}'.",
                    "nts", "test", "cli"
                );

                return Tested(await cli.Gateway.TestTimeServerAsync(host, CancellationToken));
            ...
            // at the console, which the gateway cannot tell apart.
            cli.Gateway.Log.Info(
                "Somebody at the command line asked this gateway to synchronise its time.",
                "nts", "test", "cli"
            );

            var result = await cli.Gateway.SyncTimeAsync(CancellationToken);
        ...
        /// <summary>
        /// The time servers of this gateway, switched on or not, as somebody
        ...
        private IEnumerable<String> TimeServers()
            ...
            => cli.Gateway.TimeSources.Sources.Select(source => source.Hostname.Trimmed);
        ...
        /// gateway in a German locale otherwise writes "+1,2 ms" in the middle
        /// of one.
        /// </remarks>
